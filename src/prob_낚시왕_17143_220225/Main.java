...
public class Main {
	...
	public static void main(String[] args) throws Exception{
		...
		ar = new ArrayList[R+1][C+1]; //상어가 겹칠 수 있기 때문에 M만큼의 요소를 더 줘서 3차원 배열로 사용
		for (int i = 1; i <= R; i++) {
			for (int j = 1; j <= C; j++) {
		...
		for (int i = 0; i < M; i++) {
			...
			ar[r][c].add(new shark(s,d,z,false)); //초기 상어는 0번 배열의 [r][c] 위치에 상어를 놔둠
		}
...
//			System.out.println(i + "초");
			...
			for (int j = 1; j <= R; j++) {
				if(ar[j][i].size() > 0) { // 상어를 발견했을 경우
					ans += ar[j][i].get(0).z; // 해당 상어를 잡음
//					System.out.println(j+" "+i+"의 "+ ar[j][i].get(0) + " 를 잡읍");
					ar[j][i].remove(0);
					break;
				}
			}
...
//						System.out.println(j + " " + k + " " + sh.s + " " + sh.d + " " + sh.z + " " + sh.moved);
						sh.moved = false;
						ar[j][k].remove(0);
						int dist = sh.s;
...
//						System.out.println(ar[r][c].size()+" "+r + " " +c + " " +sh.s + " " +sh.d + " " +sh.z + " " +ar[r][c].get(ar[r][c].size()-1).moved);
