...
public class Main {
	...
	public static void main(String[] args) throws Exception{
		...
		int x = 3;
		for (int t = 1; t <= 100; t++) {
			if(y < x) { // C 연산: 배열 A의 모든 열에 대해서 정렬을 수행한다. 행의 개수 < 열의 개수인 경우에 적용된다.
				//열 == 행
				y*=2;
				int maxSize_y = 0;
...
//				System.out.println();
				...
				for (int i = 0; i < ar.length; i++) {
					int[] ar_x = ar[i];
					Arrays.sort(ar_x);
//					System.out.println(Arrays.toString(ar_x));
					ArrayList<Integer> list_num = new ArrayList<Integer>();
					ArrayList<Integer> list_cnt = new ArrayList<Integer>();
					for (int j = 0; j < ar_x.length; j++) {
						if(ar_x[j] == 0) continue; // 0은 볼필요 없음
						if(!list_num.contains(ar_x[j])) { // ar[j]를 처음 발견했다면
							list_num.add(ar_x[j]); // ar[j],1의 배열값을 집어넣음
							list_cnt.add(1);
						}
						else { //이미 존재한다면 
							list_cnt.set(list_cnt.size()-1, list_cnt.get(list_cnt.size()-1)+1); // ar[j]의 발견 갯수를 추가시킴
