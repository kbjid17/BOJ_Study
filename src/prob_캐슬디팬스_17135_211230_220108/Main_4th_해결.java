...
public class Main_4th_해결 {
	static int N,M,D;
	static Queue<archer> queue = new LinkedList<>(); // 한 궁수가 탐색할 시의 방향을 따져야 하기 때문에 BFS를 사용했다.
	static int[] dy = {0,-1,0};
	static int[] dx = {-1,0,1};
	static int[] tgt;
	static int[][] ar;
	...
	static void dfs() {
		...
		for (int d = N+1; d > 0; d--) {
			...
			for (int i = 0; i < tgt.length; i++) {
				queue.offer(new archer(d,tgt[i],0));
				while(!queue.isEmpty()) {
					archer a = queue.poll();
					...
					if(a.d == 0) {
						...
						if(a.d+1 < D) {
							queue.offer(new archer(ny,nx,a.d+1));
						}
					}
					else {
						for (int j = 0; j < 3; j++) {
